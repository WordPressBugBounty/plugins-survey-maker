...
class Survey_Maker_Public {
	...
	public function ays_finish_survey( $data ){
        ...
        if($unique_id === null){
            ...
            if($survey_id === null){
                ...
                $survey_current_page_link = isset( $data['ays-survey-current_page_link'] ) && $data['ays-survey-current_page_link'] != '' ? sanitize_url( $data['ays-survey-current_page_link'] ) : "";

                $survey_current_page_title = isset( $data['ays-survey-current_page_title'] ) && $data['ays-survey-current_page_title'] != '' ? sanitize_text_field( $data['ays-survey-current_page_title'] ) : "";
                ...
                // User Website URL
                $user_website_url = (isset( $survey_user_information['user_website_url'] ) && $survey_user_information['user_website_url']  != "") ? $survey_user_information['user_website_url'] : '';
                ...
                // Current day
                $survey_current_day  = date_i18n( 'l', strtotime( sanitize_text_field( $_REQUEST['end_date'] ) ) );

                // Current month
                $survey_current_month  = date_i18n( 'F', strtotime( sanitize_text_field( $_REQUEST['end_date'] ) ) );

                // Get survey author
                $current_survey_user_data = get_userdata( $survey->author_id );
                $current_survey_author = '';
                $current_survey_author_email = '';
                $current_survey_author_nickname = '';
                $current_survey_author_display_name = '';
                $current_survey_author_website_url = '';
                $current_survey_author_registered = '';
                if ( isset( $current_survey_user_data ) && $current_survey_user_data ) {
                    ...
                    // Get survey author nickname
                    $current_survey_author_nickname = ( isset( $current_survey_user_data->data->user_nicename ) && $current_survey_user_data->data->user_nicename != '' ) ? sanitize_text_field( $current_survey_user_data->data->user_nicename ) : "";
                    // Get survey author display name
                    $current_survey_author_display_name = ( isset( $current_survey_user_data->data->display_name ) && $current_survey_user_data->data->display_name != '' ) ? sanitize_text_field( $current_survey_user_data->data->display_name ) : "";
                    // Get survey author Website URL
                    $current_survey_author_website_url = ( isset( $current_survey_user_data->data->user_url ) && $current_survey_user_data->data->user_url != '' ) ? sanitize_text_field( $current_survey_user_data->data->user_url ) : "";
                    // Get survey author Website URL
                    $current_survey_author_registered = ( isset( $current_survey_user_data->data->user_registered ) && $current_survey_user_data->data->user_registered != '' ) ? sanitize_text_field( $current_survey_user_data->data->user_registered ) : "";
                }

                if ( ! empty( $current_survey_author_website_url ) ) {
                    $current_survey_author_website_url = '<a href="'. $current_survey_author_website_url .'" target="_blank">'. $current_survey_author_website_url .'</a>';
                }
                ...
                $survey_category_title = "";
                
                $survey_cats = $survey->category_ids;
                $survey_category_ids = isset( $survey_cats ) && $survey_cats != "" ? sanitize_text_field( $survey_cats ) : '';

                if ( $survey_category_ids != '' ) {
                    $categories = self::get_survey_category_by_ids($survey_category_ids);
                    $category_names = wp_list_pluck( $categories, 'title' );

                    $survey_category_title = implode( ', ', $category_names );
                }
                ...
                $message_data = array(
                    ...
                    'survey_category_title'                 => $survey_category_title,
                    ...
                    'current_survey_author_nickname'        => $current_survey_author_nickname,
                    'current_survey_author_display_name'    => $current_survey_author_display_name,
                    'current_survey_author_website_url'     => $current_survey_author_website_url,
                    'current_survey_author_registered'      => $current_survey_author_registered,
                    'current_survey_page_link'              => $survey_current_page_link_html,
                    'current_survey_page_title'             => $survey_current_page_title,
                    ...
                    'post_author_website_url'               => $survey_current_post_author_website_url,
                    'post_author_roles'                     => $survey_current_post_author_roles,
        ...
        */
        ...
        switch( $survey_loader ){
            ...
            case 'ripple':
                ...
                break;            
            case 'snake':
                $survey_loader_html = '<div class="ays-survey-loader ays-survey-loader-snake" data-class="ays-survey-loader-snake" data-role="loader"><div></div><div></div><div></div><div></div><div></div><div></div></div>';
            break;
            case 'text':
        ...
        $current_survey_page_title = get_the_title();
        $content[] = '<input type="hidden" name="'. $this->html_name_prefix .'current_page_title" value="'. $current_survey_page_title .'">';
    ...
    public function create_sections( $sections ){
        ...
        $survey_current_post_author_website_url = get_the_author_meta( 'url', $author_id );
        $survey_current_post_author_roles  = get_the_author_meta( 'roles', $author_id );
        ...
        $survey_additional_wp_data = array(
            ...
            'survey_current_post_author_website_url' => $survey_current_post_author_website_url,
            'survey_current_post_author_roles' => $survey_current_post_author_roles,
    ...
    public static function get_survey_category_by_ids($ids){
        global $wpdb;
        ...
        $sql = "SELECT *
                FROM {$wpdb->prefix}ayssurvey_survey_categories
                WHERE id IN ($ids_sql)
